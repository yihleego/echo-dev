...
class Inspector {
  ...
  private updateCodeOutput(): void {
    ...
    this.$code.innerText = this.stringified.slice(0, this.stringified.indexOf('>') + 1).replace(/ xmlns="[^"]*"/, '');
    Prism.highlightElement(this.$code);
    /*let rect = this.$target.getBoundingClientRect();
    this.$code.style.left = rect.left + "px"
    let codeRect = this.$code.getBoundingClientRect();
    let realHeight = codeRect.height
    if (rect.top > realHeight) {
      this.$code.style.top = (rect.top - realHeight) + "px";
    } else {
      this.$code.style.top = rect.bottom + "px";
    }*/
  }
